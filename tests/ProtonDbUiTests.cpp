...
private slots:
  ...
  void badgesAppearOnWideCardsOnlyWhenEnabled() {
    QTemporaryDir dir;
    QFile file(dir.filePath("cache"));
    QVERIFY(file.open(QIODevice::WriteOnly));
    file.write(
    ...
    const auto badges = [&] {
      ...
      const auto collect = [&](auto&& self, QQuickItem* item) -> void {
        if (item->objectName() == "cardProtonBadge")
          found.append(item);
        for (auto* child : item->childItems())
          self(self, child);
      };
    ...
    preferences.setProtonDbEnabled(true);
    preferences.setProtonDbBadges(true);
    int shown = 0;
    for (auto* badge : badges()) {
      if (!badge->isVisible())
        continue;
      ++shown;
      QCOMPARE(badge->property("text").toString(), QStringLiteral("Platinum"));
    }
    QVERIFY(shown >= 1);
    QVERIFY(view.grabWindow().save("protondb-card-badges.png"));
    preferences.setProtonDbBadges(false);
    for (auto* badge : badges())
      QVERIFY(!badge->isVisible());
    service.setEnabled(false);
  }
