...
### Your own games, organization, and backups

- Add native games and desktop entries by hand, with editable arguments and
  working directory. Removing one from Omakade never deletes its files.
- Choose a preferred installation for a linked game, and add extra GOG folders.
- Customize cover, hero, and logo artwork, each resettable on its own.
- Organize several games at once, save named library filters, and pick a random
  game from the current results before deciding to play it.
- Back up personal library choices, settings, and artwork to a local archive.
  Preview it, merge or replace, and recover if a restore is interrupted.
