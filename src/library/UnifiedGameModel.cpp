...
QVariant UnifiedGameModel::data(const QModelIndex& index, int role) const {
  ...
  case GameRoles::CustomLogo: {
    // Hero and logo overrides belong in the switch, not after it: every role that reaches the
    // grouped-member code below is listed above, so a role handled only there never runs.
    const bool hero = role == GameRoles::HeroPath || role == GameRoles::CustomHero;
    const QString override =
        hero ? m_heroOverrides.value(gameKey(source)) : m_logoOverrides.value(gameKey(source));
    const bool chosen = !override.isEmpty() && QFileInfo::exists(override);
    if (role == GameRoles::CustomHero || role == GameRoles::CustomLogo) {
      return chosen;
    }
    if (chosen) {
      return localUrl(override);
